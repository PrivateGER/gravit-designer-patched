                        ...
                        if (("linear" == e.type || "radial" == e.type) && t) {
                            ...
                            if ("linear" == e.type) {
                                ...
                                if (!s) {
                                    ...
                                    var w = Math.sqrt(a._width * a._width + a._height * a._height),
                                        E = Math.cos(m),
                    ...
                    (this.makeImage = function (e, t, i, n) {
                        ...
                        if (n)
                            for (var h = 0; h + 3 < l.length; h += 4) {
                                var A = [l[h] / 255, l[h + 1] / 255, l[h + 2] / 255, l[h + 3] / 255],
                                    c = o.cmykToRGB(A);
                                ((l[h] = c[0]), (l[h + 1] = c[1]), (l[h + 2] = c[2]), (l[h + 3] = 255));
                            }
                        if (l && s.imgData.data.length == l.length) for (h = 0; h < l.length; ++h) s.imgData.data[h] = l[h];
                        ((s.imgData.data = l), s.tempContext.putImageData(s.imgData, 0, 0));
                        var p = new f();
                        return (p.setProperties(["url", "iw", "ih"], [s.canvas.toDataURL(), e, t]), p);
                    }),
                    (this.getClipPath = function (e) {
                        ...
                        for (var i = 0; i < e.length; i++)
                            (t = this.path2GPath(e[i])) &&
                                ...
                                i > 0 ? this.appendItem(t, this.clipGPathes[i - 1]) : this.appendItem(t),
                        ...
                        for (var t = new u(), i = 0; i < e.length; i++)
                            for (var n = 0; n < e[i].length; n++)
                                if ("curveto" == e[i][n].type)
                                    (t.addVertex(a.Command.Curve2, e[i][n].coord3.x, e[i][n].coord3.y),
                                        t.addVertex(a.Command.Curve2, e[i][n].coord1.x, e[i][n].coord1.y),
                                        t.addVertex(a.Command.Curve2, e[i][n].coord2.x, e[i][n].coord2.y));
                                else if ("lineto" == e[i][n].type) t.addVertex(a.Command.Line, e[i][n].coord.x, e[i][n].coord.y);
                                else if ("closepath" == e[i][n].type) t.addVertex(a.Command.Close, e[i][n].coord.x, e[i][n].coord.y);
                                else if ("moveto" == e[i][n].type) t.addVertex(a.Command.Move, e[i][n].coord.x, e[i][n].coord.y);
                                else if ("arc" == e[i][n].type) {
                                    var r = e[i][n],
                                        o = new d().scaled(r.radius.x, r.radius.y).translated(r.center.x, r.center.y);
                                    (h = new _()).setProperties(
                                        ["sa", "ea", "etp", "trf"],
                                        [(r.angle1 + 2 * Math.PI) % (2 * Math.PI), (r.angle2 + 2 * Math.PI) % (2 * Math.PI), _.Type.Arc, o]
                                    );
                                    var l = new a();
                        ...
                        drawImage: function (e, t, i, n, r, a, s) {
                            ...
                            if (s) {
                                ...
                                for (var p = 0, u = 0; p + 4 < i.length; p += 5, u += 4) {
                                    var g = [i[p] / 255, i[p + 1] / 255, i[p + 2] / 255, i[p + 3] / 255],
                                        m = o.cmykToRGB(g);
                                    ((c[u] = m[0]), (c[u + 1] = m[1]), (c[u + 2] = m[2]), (c[u + 3] = i[p + 4]));
                                }
                            }
                            if (c && A.imgData.data.length == c.length) for (p = 0; p < c.length; ++p) A.imgData.data[p] = c[p];
                            ((A.imgData.data = c), A.tempContext.putImageData(A.imgData, 0, 0));
                            var y = new f();
                            y.setProperties(["url", "iw", "ih"], [A.canvas.toDataURL(), e, t]);
                            ...
                            if (b)
                                for (var C = 0; C < b.length; ++C) {
                                    ...
                                    if ((B.setProperties(["_pt"], [r]), 2 == a)) {
                                        ...
                                        var T = y || 3;
                                        B.setProperties(["_bw", "_blc", "_blj", "_bds", "_bml"], [u, x, P, S, T]);
                                    }
                                    ...
                                    (R &&
                                        ...
                                        (D = (R._openTypeFont.ascender / R._openTypeFont.unitsPerEm) * n),
