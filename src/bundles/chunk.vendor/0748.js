                    ...
                    if (!(this._current && p.isEqualEps(this._current.getX(), e.getX()) && p.isEqualEps(this._current.getY(), e.getY()))) {
                        if ((this._areaStartedPos || (this._areaStartedPos = e), this._trf)) {
                            ...
                            (i && i.length && (this._processedVertices = i[0]),
                                this._current && (this._current = this._trf.mapPoint(this._current)),
                                (this._trf = null),
                        ...
                        if (null == this._current || n >= 1) {
                            if (20 === this._currentPoints.length || (this._currentPoints.length > 4 && this._cdist > 400)) {
                                ...
                                if (a > 0) {
                                    ...
                                    ((this._firstTangent = this._currentPoints[0].subtract(h)),
                                        (this._lastTangent = this._currentPoints[this._currentPoints.length - 1].subtract(e)),
                                ...
                                var d,
                                    g = this._fitter.fitCurve(this._currentPoints, 20, this._firstTangent, this._lastTangent);
                                if (o && g) {
                                    ((d = g[0]), this._processedVertices.addVertex(A.Command.Move, d[0].getX(), d[0].getY()));
                                    for (var f = 0; f < g.length; f++)
                                        ((d = g[f]),
                                            this._processedVertices.addVertex(A.Command.Curve2, d[3].getX(), d[3].getY()),
                                            this._processedVertices.addVertex(A.Command.Curve2, d[1].getX(), d[1].getY()),
                                            this._processedVertices.addVertex(A.Command.Curve2, d[2].getX(), d[2].getY()));
                                } else if (g)
                            ...
                            (this._vertexContainer.addVertex(
                                0 == this._vertexContainer.getCount() ? A.Command.Move : A.Command.Line,
                                e.getX(),
                                e.getY()
                            ),
