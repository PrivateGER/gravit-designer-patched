    ...
    expressStaticGzip(publicDir, {
        ...
        serveStatic: {
            ...
            setHeaders: (res, filePath) => {
                ...
                } else if (plainPath.startsWith(path.join(publicDir, "assets") + path.sep)) {
                    // Versioned assets (requested with ?v=...) can be cached aggressively.
                    res.setHeader("Cache-Control", "public, max-age=604800, immutable");
                } else if (path.dirname(plainPath) === publicDir && /\.(js|css)$/.test(plainPath)) {
                    // App-shell bundles/styles are referenced from index.html without
                    // cache busters; force revalidation (cheap 304s) so a deploy rolls
                    // out immediately. Long-term caching belongs to the SW precache.
                    res.setHeader("Cache-Control", "no-cache");
                }
...
wss.on("connection", (ws, req) => {
    ...
    // Without an 'error' listener a single malformed frame (unmasked, bad
    // UTF-8, oversized) would throw an unhandled 'error' event and crash the
    // whole process — a trivial remote DoS.
    ...
    ws.on("message", (message) => {
        ...
        // The bundled client's keepalive sends the raw string "ping" every 50s
        // (chunk.vendor module 0957), not JSON — answer it before parsing.
        if (raw === "ping") {
